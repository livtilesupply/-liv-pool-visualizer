...
const checks={
 ...
 html:html.includes('id="photo"')&&html.includes('id="demo"')&&html.includes('/app.js?v=final8')&&html.includes('data:image/jpeg;base64,/9j/'),
...
http.createServer((req,res)=>{
 ...
 if(p==='/api/health'){
   const b=Buffer.from(JSON.stringify({ok,build:'browser-tested-final8',apiCreditsRequired:false,checks,heroBytes:cache['/hero.jpg'].length}));
 ...
 res.end(b);
}).listen(port,'0.0.0.0',()=>console.log('LIV browser-tested final8 running on '+port));